...
export default function Map() {
  ...
  const [L, setL] = useState(null); // Leaflet only on client
  ...
  /* ✅ Load markers from localStorage (client only) */
  useEffect(() => {
  ...
  function getIconPerCategory(name) {
    switch (name) {
      ...
      case "Véhicule":
        return `<i class="bi bi-car-front-fill fs-3"></i>`;
      case "Ressources":
        return `<i class="bi bi-database-fill fs-3"></i>`;
      case "Stockage":
        return `<i class="bi bi-box-seam fs-3"></i>`;
      case "Capteurs":
        return `<i class="bi bi-wifi fs-3"></i>`;
      default:
        return "";
    }
  }
  ...
  const createHtmlIcon = (label) => {
    ...
    return L.divIcon({
      className: "",
      html: `
        <div class="rounded-circle thm-bg d-flex align-items-center justify-content-center"
             style="width:40px;height:40px;box-shadow:0 2px 6px rgba(0,0,0,.3)">
  ...
  /* ⛔ Prevent render until Leaflet is ready */
  if (!L) return null;
  ...
  return (
    <div className="thm-bg-dark p-4">
        ...
        <div className="d-flex justify-content-between">
          <div className="d-flex">
            ...
            <div className="pt-3 ms-2">
              <p className="fs-3 no-wrap">Détails de la carte</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* CONTROLS */}
      <div className="container thm-bg-light p-3 rounded-3 mt-4 thm-shadow-m">
        <div className="row">
          {categories.map((c) => (
              ...
              <div className="form-check form-switch d-flex align-items-center justify-content-between">
                <span
                  dangerouslySetInnerHTML={{
                    __html: getIconPerCategory(c.name),
                  }}
                />
      ...
      {/* MAP */}
      <div className="container thm-bg-light p-3 mt-4">
